...
func TestCheckPkScriptStandard(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		if len(pk.PubKey().SerializeCompressed()) != 33 {
			panic("fuck")
		}
	...
	for _, test := range tests {
		...
		if (test.isStandard && got != nil) ||
			...
			t.Errorf("TestCheckPkScriptStandard test '%s' sc:%s failed\n%x\n",
				test.name, scriptClass, script)
			return
		}
	}
}
...
func TestDust(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Zero value is dust.
			"zero value with default minimum relay tx fee",
			wire.TxOut{0, pkScript},
			defaultMinRelayTxFee,
			true,
		},
