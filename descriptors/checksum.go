...
const (
	...
	checksumInputCharset = "0123456789()[],'/*abcdefgh@:$%{}IJKLMNOPQRSTU" +
		"VWXYZ&+-.;<=>?!^_|~ijklmnopqrstuvwxyzABCDEFGH`#\"\\ "
...
func descriptorChecksum(descriptor string) string {
	...
	for range checksumLength {
		c = checksumPolyMod(c, 0)
	}
	c ^= 1

	var sb strings.Builder
	for j := range checksumLength {
		shift := uint(5 * (checksumLength - 1 - j))
		sb.WriteByte(checksumCharset[(c>>shift)&31])
	}
	return sb.String()
}
