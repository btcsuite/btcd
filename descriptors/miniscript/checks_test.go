...
func TestStackSize(t *testing.T) {
	...
	overLimit, err := ParseInsane(andVChain(100), P2WSH)
	require.NoError(t, err)
...
func TestLegacyContext(t *testing.T) {
	...
	require.ErrorContains(
		t, err, "larger than the maximum script size of 520 in the "+
			"Legacy context",
	)
...
func TestStackSizeLimit(t *testing.T) {
	...
	require.Less(
		t, atWitnessLimit.maxWitnessSize()+
			atWitnessLimit.maxExecStackSize(), maxStackSize,
	)
	...
	for _, ctx := range []Context{P2TR, P2WSH} {
		...
		require.ErrorContainsf(
			...
			"larger than the consensus limit of 1000",
			"context %v", ctx,
		)
	}
