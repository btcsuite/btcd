...
func combineTimelocks(k int, subs ...timelockInfo) timelockInfo {
	...
	for _, t := range subs {
		...
		// Even alternatives retain all possible lock kinds, and a
		// conflict already inside one child remains a conflict under
		// disjunction.
...
func computeTimelocks(node *AST) (*AST, error) {
	...
	case f_andor:
		...
		node.timelock = combineTimelocksOr(combineTimelocksAnd(
			node.args[0].timelock, node.args[1].timelock,
