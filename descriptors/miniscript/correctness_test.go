...
// TestThreshOneArgProperty verifies the `o` (one) type property of thresh,
// which was previously always computed as false. rust-miniscript computes it as
// true for a single-sub threshold whose sub reads exactly one stack element.
func TestThreshOneArgProperty(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, err)
	require.True(
		t, single.props.o, "thresh(1,pk(A)) must have the o property",
	)

	// These two pk checks consume two elements, so their threshold lacks o.
...
func TestVerifyWrapperCollapse(t *testing.T) {
	...
	node, err := Parse(
		"and_v(v:or_d(c:pk_h(A),multi(2,B,C,D)),sha256("+
			h+"))",
		P2WSH,
	)
	require.NoError(t, err)
	...
	require.Contains(
		t, disasm, "OP_CHECKSIG OP_IFDUP",
		"inner checksig must stay non-VERIFY",
	)
	require.Contains(
		t, disasm, "OP_CHECKMULTISIG OP_ENDIF OP_VERIFY",
		"inner checkmultisig must stay non-VERIFY; the v: adds "+
			"OP_VERIFY after the ENDIF",
	)
	...
	require.NoError(t, err)
	require.Contains(
		t, disasm2, "OP_CHECKMULTISIGVERIFY",
