...
func TestCombineTimelocks(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(
				t, tc.wantCombination, got.containsCombination,
			)
		})
	}
...
func TestComputeTimelocksLeaf(t *testing.T) {
	...
	require.Equal(t, timelockInfo{cltvWithHeight: true}, leaf(f_after, 100))
	require.Equal(t, timelockInfo{cltvWithTime: true}, leaf(
		f_after, threshold,
	))
	require.Equal(t, timelockInfo{csvWithHeight: true}, leaf(f_older, 100))
	require.Equal(t, timelockInfo{csvWithTime: true}, leaf(
		f_older, secondsBit|100,
	))
}
