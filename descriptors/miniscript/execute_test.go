...
// through the script engine. This checks that every produced witness in this
// corpus spends the generated script, exercising the full
	...
	for scanner.Scan() {
		...
		sign := func(pubKey []byte) ([]byte, bool) {
			for name, pub := range pubKeys {
				...
				return append(sig.Serialize(), byte(
					txscript.SigHashAll,
				)), true
			}
			return nil, false
		...
		require.LessOrEqualf(
			t, peak, max(2, bound),
			"execution of %s peaked at %d stack elements, more "+
				"than the %d accounted for",
			expr, peak, bound,
		)
	}
...
func checkSatisfactionSizes(t *testing.T, file string, ctx Context) {
	...
	checked, exact := 0, 0
	for scanner.Scan() {
		expr, _, _ := strings.Cut(scanner.Text(), "\t")
		...
		require.LessOrEqualf(
			t, produced, stated,
			"satisfaction of %s is %d bytes, more than the "+
				"computed maximum of %d",
