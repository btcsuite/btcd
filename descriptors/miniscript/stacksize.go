...
// computeStackSize derives canonical witness-element bounds bottom-up. These
// counts exclude the revealed script and any Taproot control block.
func computeStackSize(node *AST) (*AST, error) {
	// Empty witnesses are possible paths with zero elements. Impossible
	// paths carry a separate flag so alternatives cannot select them.
	...
	case f_and_v:
		// and_v has no canonical dissatisfaction. A solution that
		// satisfies X and dissatisfies Y is non-canonical and is not
	...
	case f_thresh:
		...
		for i, arg := range node.args[1:] {
			...
			subSat[i] = arg.stackSize.sat
			subDsat[i] = arg.stackSize.dsat
		}

		// Satisfaction satisfies exactly k sub expressions and
		// dissatisfies the rest, and its witness element count is the
		// sum of the chosen sub expressions' counts. We need the worst
		// case over all such choices, which the selection computes
		// without enumerating them.
		sat := newThreshSelection(k, subSat, subDsat).maxSum()
