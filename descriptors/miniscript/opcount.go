package miniscript

import "fmt"

// maxInt represents a possible path's cost. The zero value marks an impossible
// path, which is distinct from a valid path with zero cost.
type maxInt struct {
	valid bool
	value int
}

// and adds costs of paths that must both be possible, propagating
// impossibility.
func (m maxInt) and(b maxInt) maxInt {
	if !m.valid || !b.valid {
		return maxInt{}
	}

	return maxInt{
		valid: true,
		value: m.value + b.value,
	}
}

// or selects the largest possible alternative, ignoring impossible paths.
func (m maxInt) or(b maxInt) maxInt {
	if !m.valid {
		return b
	}
	if !b.valid {
		return m
	}

	return maxInt{
		valid: true,
		value: max(m.value, b.value),
	}
}

// ops separates unconditional opcode cost from the path-dependent number of
// keys charged by executed CHECKMULTISIG operations.
type ops struct {
	// count is the number of non-push opcodes.
	count int

	// dsat is the number of keys in possibly executed
	// OP_CHECKMULTISIG(VERIFY)s to dissatisfy.
	dsat maxInt

	// sat is the number of keys in possibly executed
	// OP_CHECKMULTISIG(VERIFY)s to satisfy.
	sat maxInt
}

// computeOpCount derives the non-push opcode bound for canonical satisfactions
// and dissatisfactions from already-analyzed children.
func computeOpCount(node *AST) (*AST, error) {
	// An impossible path must not be confused with a possible path that
	// executes no CHECKMULTISIG operations. Combinators propagate this
	// flag.
	zero := maxInt{valid: true, value: 0}
	invalid := maxInt{valid: false}
	switch node.identifier {
	case f_0:
		node.opCount = ops{0, zero, invalid}

	case f_1:
		node.opCount = ops{0, invalid, zero}

	case f_pk_k:
		node.opCount = ops{0, zero, zero}

	case f_pk_h:
		node.opCount = ops{3, zero, zero}

	case f_older, f_after:
		node.opCount = ops{1, invalid, zero}

	case f_sha256, f_hash256, f_ripemd160, f_hash160:
		node.opCount = ops{4, zero, zero}

	case f_andor:
		x, y, z := node.args[0], node.args[1], node.args[2]
		node.opCount = ops{
			3 + x.opCount.count + y.opCount.count + z.opCount.count,
			z.opCount.dsat.and(x.opCount.dsat),
			y.opCount.sat.and(x.opCount.sat).or(z.opCount.sat.and(
				x.opCount.dsat,
			)),
		}

	case f_and_v:
		x, y := node.args[0], node.args[1]
		node.opCount = ops{
			x.opCount.count + y.opCount.count,
			invalid,
			y.opCount.sat.and(x.opCount.sat),
		}

	case f_and_b:
		x, y := node.args[0], node.args[1]
		node.opCount = ops{
			1 + x.opCount.count + y.opCount.count,
			y.opCount.dsat.and(x.opCount.dsat),
			y.opCount.sat.and(x.opCount.sat),
		}

	case f_or_b:
		x, z := node.args[0], node.args[1]
		node.opCount = ops{
			1 + x.opCount.count + z.opCount.count,
			z.opCount.dsat.and(x.opCount.dsat),
			z.opCount.dsat.and(x.opCount.sat).or(z.opCount.sat.and(
				x.opCount.dsat,
			)),
		}

	case f_or_c:
		x, z := node.args[0], node.args[1]
		node.opCount = ops{
			2 + x.opCount.count + z.opCount.count,
			invalid,
			x.opCount.sat.or(z.opCount.sat.and(x.opCount.dsat)),
		}

	case f_or_d:
		x, z := node.args[0], node.args[1]
		node.opCount = ops{
			3 + x.opCount.count + z.opCount.count,
			z.opCount.dsat.and(x.opCount.dsat),
			x.opCount.sat.or(z.opCount.sat.and(x.opCount.dsat)),
		}

	case f_or_i:
		x, z := node.args[0], node.args[1]
		node.opCount = ops{
			3 + x.opCount.count + z.opCount.count,
			x.opCount.dsat.or(z.opCount.dsat),
			x.opCount.sat.or(z.opCount.sat),
		}

	case f_thresh:
		k := node.args[0].num
		n := len(node.args) - 1

		count := 0

		// The thresh is dissatisfiable iff all sub expressions are
		// dissatisfiable, which is done by dissatisfying every one of
		// them in order. The op count therefore starts at zero and
		// accumulates each sub expression's dissatisfaction cost; if
		// any sub is not dissatisfiable, the .and below propagates the
		// invalid value.
		dsat := zero
		subSat := make([]maxInt, n)
		subDsat := make([]maxInt, n)
		for i, arg := range node.args[1:] {
			count += arg.opCount.count + 1
			dsat = dsat.and(arg.opCount.dsat)
			subSat[i], subDsat[i] = arg.opCount.sat, arg.opCount.dsat
		}

		// The satisfaction satisfies exactly k sub expressions and
		// dissatisfies the rest, and its op count is the sum of the
		// chosen sub expressions' counts. We need the worst case over
		// all such choices, which the selection computes without
		// enumerating them.
		sat := newThreshSelection(int(k), subSat, subDsat).maxSum()

		node.opCount = ops{count, dsat, sat}

	case f_multi:
		n := len(node.args) - 1
		node.opCount = ops{
			1,
			maxInt{valid: true, value: n},
			maxInt{valid: true, value: n},
		}

	case f_multi_a:
		// multi_a only occurs in the Tapscript context, which has no op
		// count limit, so the op count is never actually consulted for
		// it. It contributes no executed CHECKMULTISIG keys.
		node.opCount = ops{0, zero, zero}

	case f_wrap_a:
		x := node.args[0]
		node.opCount = ops{
			2 + x.opCount.count,
			x.opCount.dsat,
			x.opCount.sat,
		}

	case f_wrap_s, f_wrap_c, f_wrap_n:
		x := node.args[0]
		node.opCount = ops{
			1 + x.opCount.count,
			x.opCount.dsat, x.opCount.sat,
		}

	case f_wrap_d:
		x := node.args[0]
		node.opCount = ops{
			3 + x.opCount.count,
			zero, x.opCount.sat,
		}

	case f_wrap_v:
		x := node.args[0]
		opVerify := 0
		if !node.args[0].props.canCollapseVerify {
			opVerify = 1
		}
		node.opCount = ops{
			opVerify + x.opCount.count, invalid, x.opCount.sat,
		}

	case f_wrap_j:
		x := node.args[0]
		node.opCount = ops{4 + x.opCount.count, zero, x.opCount.sat}

	default:
		return nil, fmt.Errorf("unknown identifier: %s",
			node.identifier)
	}

	return node, nil
}
