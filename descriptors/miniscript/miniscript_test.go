...
func TestVectors(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		for i, line := range lines {
			...
			require.NoError(t, checkMiniscript(
...
// testSignFn returns a DER signature without its sighash byte, if available.
type testSignFn func(pubKey []byte, hash []byte) (signature []byte,
	available bool)

// testRedeem constructs and executes a P2WSH spend using the supplied secrets.
// It returns parse, construction and execution errors for negative test cases.
func testRedeem(t *testing.T, miniscript string,
	...
	node, err := Parse(miniscript, P2WSH)
	if err != nil {
		return err
	}
	err = node.IsSane()
	...
	// The actual witness (excluding the witness script, which is appended
	// below) must never exceed the statically computed maximum.
	require.LessOrEqualf(
		t, len(witness), node.maxWitnessSize(),
		"witness for %s has more elements than the computed maximum",
		miniscript,
	)
...
// RedeemTestVectors contains concrete secrets and the spends that use them.
type RedeemTestVectors struct {
	Identifiers map[string]string `json:"identifiers"`
	TestCases   []RedeemTestCase  `json:"test_cases"`
}

// RedeemTestCase describes a spend and the signing material available to it.
...
func TestRedeem(t *testing.T) {
	...
	// A hash-locked spend must reach the preimage callback with the actual
	// opcode's algorithm name. The imported cases can take alternate paths,
	// so force HASH160 here to catch a callback mislabeled as RIPEMD160.
	t.Run("hash160 preimage", func(t *testing.T) {
		err := testRedeem(
			t, "and_v(v:pk(key_1),hash160(h160_1))", lookupVar, 0,
			sign(true, false, false), preimage(true),
		)
		require.NoError(t, err)
	})
