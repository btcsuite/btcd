...
var (
	// errImpossibleSatisfaction is returned by the satisfaction-size
	// accessors when the expression can never be satisfied (e.g. the "0"
	// combinator).
	errImpossibleSatisfaction = errors.New("miniscript cannot be satisfied")
)
...
func computeSatSize(node *AST) (*AST, error) {
	...
	case f_wrap_d:
		...
		// d: adds a "1" selector element to the child's satisfaction,
		// two bytes with its length prefix, and is dissatisfied by a
		// single empty element, one byte.
		sat := x.satSize.sat
		if sat.valid {
			sat = satBound{
				valid: true,
