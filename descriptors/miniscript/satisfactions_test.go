...
func TestCheckOlder(t *testing.T) {
	...
	tests := []struct {
		...
		// A transaction version is a signed field, so a negative one
		// must not pass the "at least 2" comparison either.
...
func TestSatisfyIncompleteSatisfier(t *testing.T) {
	...
	node, err = ParseInsane(
		"sha256(926a54995ca48600920a19bf7bc502ca5f2f7d07e6f804c4f00eb"+
			"f0325084dbc)",
		P2WSH,
	)
	require.NoError(t, err)
