...
const (
	...
	// schnorrKeySpendSig is the assumed size of a Schnorr key-spend
	// signature push (a 64-byte signature plus a one-byte sighash byte and
	...
	// of a script element applies to it. This mirrors the limit the
	// miniscript Legacy context enforces for a miniscript redeem script.
	maxRedeemScriptSize = 520

	// maxScriptSigSize is the maximum size in bytes of a standard
	// scriptSig, which holds both the redeem script and its satisfaction.
	// This is a standardness rule.
	maxScriptSigSize = 1650
)

var (
	// errUnsupportedInner is returned when a descriptor node's weight
	// cannot be estimated because its inner script kind is not supported.
	errUnsupportedInner = errors.New(
		"unsupported inner script for weight estimation",
	)
)

// MaxWeightToSatisfy returns an upper bound on the additional input weight, in
// weight units, relative to an empty scriptSig and witness. It excludes the
// outpoint, sequence and transaction-wide witness marker/flag. Signature sizes
// include a sighash byte and assume low-S ECDSA signatures.
//
// It mirrors rust-miniscript's max_weight_to_satisfy and returns an error if
// the descriptor cannot be satisfied. Unlike Plan.SatisfactionWeight, it uses
// the largest path without considering available assets and subtracts the
// empty serialization prefixes.
...
func (d *Descriptor) maxWeight(n *node) (uint64, error) {
	...
	case nodePkh:
		// Legacy scriptSig: <sig> <pubkey>. The key push is 66 bytes
		// rather than 34 if the descriptor used an uncompressed key,
		// which is valid in this pre-segwit position.
		scriptSig := ecdsaSigSize + n.keys[0].pushLen()
	...
	default:
		// A bare descriptor: a top-level pk() or a bare multisig. Its
		// satisfaction lives entirely in the scriptSig.
		_, satSize, _, err := satInfo(n)
		if err != nil {
			return 0, err
		}
...
func (d *Descriptor) maxWeightWsh(sub *node) (uint64, error) {
	scriptSize, satSize, satElems, err := satInfo(sub)
	if err != nil {
		return 0, err
	}
...
func (d *Descriptor) maxWeightTr(n *node) (uint64, error) {
	...
	err := n.tapTree.forEachLeaf(0, func(leaf *node, depth int) error {
		scriptSize, satSize, satElems, err := satInfo(leaf)
		if err != nil {
			return err
		}
		...
		controlBlock := tapControlBlockBase + depth*tapControlBlockNode
...
func satInfo(n *node) (scriptSize, satSize, satElems int, err error) {
	switch n.kind {
	case nodeMs:
		ast := n.clonedMsAST()
	...
	case nodeMulti, nodeSortedMulti:
		...
		// Each key contributes its own push, whose size depends on the
		// serialization its position requires.
		keyPushes := 0
		for _, key := range n.keys {
			keyPushes += key.pushLen()
		}
		scriptSize = multiNumCost(k, numKeys) + keyPushes + 1
	...
	case nodePkh:
		...
		// EQUALVERIFY CHECKSIG. The script only holds the key hash, but
		// the satisfaction pushes the key itself.
		scriptSize = 25
		satSize = n.keys[0].pushLen() + ecdsaSigSize
		satElems = 2 + 1
		return scriptSize, satSize, satElems, nil
