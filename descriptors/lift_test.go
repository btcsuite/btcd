...
func TestFromMiniscriptPolicy(t *testing.T) {
	...
	require.Equal(
		t, SemanticPolicyTypeTrivial,
		fromMiniscriptPolicy(
			&miniscript.Policy{Type: miniscript.PolicyTrivial},
		).Type,
	)
	require.Equal(
		t, SemanticPolicyTypeUnsatisfiable,
		fromMiniscriptPolicy(
			&miniscript.Policy{
				Type: miniscript.PolicyUnsatisfiable,
			},
		).Type,
	)
...
// TestSemanticPolicyStringIncomplete checks that a policy which is not the
// output of Lift renders the optional fields it left unset as a placeholder
// instead of panicking. The type is exported with JSON tags, so a consumer can
// unmarshal a document that omits them.
func TestSemanticPolicyStringIncomplete(t *testing.T) {
	t.Parallel()

	// A nil policy, including one nested in a threshold, is a valid
	// receiver.
	var nilPolicy *SemanticPolicy
	require.Equal(t, "<missing>", nilPolicy.String())
	...
	for _, tc := range []struct {
		...
		name: "key without key",
		json: `{"type":"key"}`,
		want: "pk(<missing>)",
	}, {
		name: "after without locktime",
		json: `{"type":"after"}`,
		want: "after(<missing>)",
	}, {
		name: "older without locktime",
		json: `{"type":"older"}`,
		want: "older(<missing>)",
	}, {
		name: "sha256 without hash",
		json: `{"type":"sha256"}`,
		want: "sha256(<missing>)",
	}, {
		name: "hash160 without hash",
		json: `{"type":"hash160"}`,
		want: "hash160(<missing>)",
	}, {
		name: "thresh without threshold",
		json: `{"type":"thresh","policies":[{"type":"trivial"},` +
			`{"type":"key","key":"A"}]}`,
		want: "thresh(<missing>,TRIVIAL,pk(A))",
	}, {
		name: "thresh with a nil sub policy",
		json: `{"type":"thresh","threshold":1,"policies":[null,null]}`,
		want: "or(<missing>,<missing>)",
	}, {
		name: "unknown type",
		json: `{"type":"nonsense"}`,
		want: "<unknown>",
...
// TestLiftHashCommitments checks that lifting a parsed descriptor preserves
// literal hash commitments without first deriving its keys or compiling it.
func TestLiftHashCommitments(t *testing.T) {
	t.Parallel()

	// Hash commitments are already concrete in a descriptor, unlike keys
	// with wildcards. Lifting must preserve them even before ApplyVars
	// runs.
	for _, fragment := range []struct {
		...
		{"sha256", testSha256},
		{"hash256", testSha256},
		{"ripemd160", testHash160},
		{"hash160", testHash160},
	} {

		t.Run(fragment.name, func(t *testing.T) {
			hash := fragment.name + "(" + fragment.hash + ")"
			d, err := NewDescriptor(
				"wsh(and_v(v:pk(" + basicTestXpub +
					")," + hash + "))",
			)
