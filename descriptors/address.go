...
// derivation index for the given network. params must be non-nil and
// multipathIndex must be below MultipathLen(). A wildcard's derivation index
// must be below 2^31; otherwise it is ignored. Bare descriptors have no
// address.
func (d *Descriptor) AddressAt(params *chaincfg.Params, multipathIndex,
	...
	addr, err := d.address(d.root, params, multipathIndex, derivationIndex)
	if err != nil {
		return "", err
	}
...
func (d *Descriptor) address(n *node, params *chaincfg.Params, mp,
	...
	case nodePkh:
		...
		return address.NewAddressPubKeyHash(address.Hash160(pk), params)
...
func (d *Descriptor) multiScript(n *node, mp, idx uint32) ([]byte, error) {
	// Derive before sorting: BIP67 orders concrete public keys, not their
	// descriptor expressions, and that order can vary by derivation index.
	...
	if n.kind == nodeSortedMulti {
		slices.SortFunc(pubKeys, bytes.Compare)
	}
...
func (d *Descriptor) taprootOutputKey(n *node,
	mp, idx uint32) (*btcec.PublicKey, error) {
...
// index, without a length prefix. P2WPKH returns its P2PKH scriptCode; P2SH
// wrappers are removed. Taproot has no scriptCode and returns an error.
// The index constraints are the same as for AddressAt.
func (d *Descriptor) ScriptCodeAt(
	multipathIndex, derivationIndex uint32) ([]byte, error) {
...
func (d *Descriptor) scriptCode(n *node, mp, idx uint32) ([]byte, error) {
	...
	// bare multisigs of BIP383.
	case nodePkh, nodePk, nodeMulti, nodeSortedMulti:
		return d.innerScript(n, mp, idx)
