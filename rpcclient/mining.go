...
func (c *Client) GenerateToAddressAsync(numBlocks uint32, address string) FutureGenerateToAddressResult {
	cmd := btcjson.NewGenerateToAddressCmd(numBlocks, address)
	return c.sendCmd(cmd)
}

// GenerateToAddress generates numBlocks blocks with subsidy for the specified
// address and returns their hashes.
