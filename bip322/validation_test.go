...
func TestVerifyMessagePoFWitnessUtxoHandling(t *testing.T) {
	...
	// A legacy PoF input that carries only a witness UTXO is rejected up
	// front by validateUtxoCorrectness during serialization: a non-segwit
	// input must supply a non-witness UTXO, so no signature is produced.
...
func TestVerifyMessagePofRejectsOversizeWitness(t *testing.T) {
	message := []byte("probe")

	pkScriptOpTrue, _, _ := opTrueChallenge(t)
	...
	require.ErrorContains(t, err, "too many witness items")
	require.False(t, valid)
}
