...
func GetDustThreshold(txOut *wire.TxOut) int64 {
	// Pay-to-Anchor outputs have a fixed dust threshold of 240 satoshis
	// per BIP 433 ("As an implementation detail, dust threshold for P2A
	// is set at 240 satoshis by default"). This is intentionally
	// independent of minRelayTxFee — see IsDust below for the matching
	// fee-independent gate.
...
func IsDust(txOut *wire.TxOut, minRelayTxFee btcutil.Amount) bool {
	...
	// Pay-to-Anchor outputs are gated against a fixed 240 sat threshold
	// per BIP 433, regardless of the configured minRelayTxFee. This is
	// an intentional fixed-policy exception, not a fee-derived dust
	// computation.
	if txscript.IsPayToAnchorScript(txOut.PkScript) {
