...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, relayTxFeeRate btcutil.Amount) int64 {
	...
	// minimum Satoshis.
	minFee := (serializedSize * int64(relayTxFeeRate)) / 1000

	// In the case that size*rate < 1000 we use relayTxFeeRate in case it is > 0.
	if minFee == 0 {
		minFee = int64(relayTxFeeRate)
	}
