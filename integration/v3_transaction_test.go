...
func testV3TransactionMining(t *testing.T, r *rpctest.Harness) {
	...
	// Verify by checking the block contains our transaction. The
	// transaction should have been removed from mempool and included in
	// block.
	mempool, err = r.Client.GetRawMempool()
	require.NoError(t, err)
	require.NotContains(t, mempool, v3TxHash)
...
func TestV3TransactionChaining(t *testing.T) {
	...
	addr1, err := r.NewAddress()
	require.NoError(t, err)

	script1, err := txscript.PayToAddrScript(addr1)
	require.NoError(t, err)
	...
	// Create and submit first v3 transaction.
	tx1, err := r.CreateTransaction(
		[]*wire.TxOut{output1}, 10, true, rpctest.WithTxVersion(3),
	)
	require.NoError(t, err)

	tx1Hash, err := r.Client.SendRawTransaction(tx1, true)
	require.NoError(t, err)
	require.NotNil(t, tx1Hash)
	...
	// Now create a second v3 transaction that spends from the first. Note:
	// This tests that the wallet can properly handle spending from v3
	// transaction outputs.
	...
	// Create second v3 transaction.
	tx2, err := r.CreateTransaction(
		[]*wire.TxOut{output2}, 10, true, rpctest.WithTxVersion(3),
	)
	require.NoError(t, err)

	tx2Hash, err := r.Client.SendRawTransaction(tx2, true)
	require.NoError(t, err)

	// Verify both transactions work correctly in the chain.
	mempool, err := r.Client.GetRawMempool()
	require.NoError(t, err)
	require.Contains(t, mempool, tx2Hash)
	...
	// Verify the second v3 transaction is no longer in mempool (was mined).
	mempool, err = r.Client.GetRawMempool()
	require.NoError(t, err)
