...
type Peer struct {
	...
	invVectsTimerMtx sync.Mutex
	invVectsTimer    *time.Timer
...
// queueInvVects gathers all the pending inventory vectors into one or more inv
// messages and then queues them to be sent to the remote peer.  This method is
// called periodically via a timer.
func (p *Peer) queueInvVects() {

	invMsg := wire.NewMsgInvSizeHint(uint(len(p.invVects)))
	for invVect := range p.invVects {
	...
	p.invVectsTimerMtx.Lock()
	if p.invVectsTimer != nil {
		p.invVectsTimer.Reset(trickleTimeout)
	}
	p.invVectsTimerMtx.Unlock()
}
...
func (p *Peer) Disconnect() {
	...
	// prevent a race if the timers functions are concurrently executing.
	p.pingTickerTimerMtx.Lock()
	if p.pingTickerTimer != nil {
...
func (p *Peer) start() error {
	...
	p.invVectsTimerMtx.Lock()
	p.invVectsTimer = time.AfterFunc(trickleTimeout, p.queueInvVects)
	p.invVectsTimerMtx.Unlock()
