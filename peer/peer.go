...
// recvMsgsubscription consists of two channels for a caller that subscribed to
// receive all the messages from a given peer.  The msgChan is used for sending
// the messages and the quit chan is used for cancelling the subscription.
type recvMsgsubscription struct {
	msgChan chan wire.Message
	quit    chan struct{}
}

// SubscribeRecvMsg adds an OnRead subscription to the peer.  All bitcoin
...
out:
	for atomic.LoadInt32(&p.disconnect) == 0 {
		...
		for sub := range p.subscribers {
			...
			go func(subscription recvMsgsubscription,
				quit chan struct{}) {

				select {
				case subscription.msgChan <- rmsg:
