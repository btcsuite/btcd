	...
	tests := []struct {
	...
	}{
		{
			...
			func() (*peer.Peer, *peer.Peer, error) {
				...
				wg.Add(2)
				go func() {
					inPeer, outPeerErr = peer.NewInboundPeer(peerCfg, inConn)
					wg.Done()
				}()
				go func() {
					outPeer, outPeerErr = peer.NewOutboundPeer(peerCfg, outConn)
					wg.Done()
				}()
				wg.Wait()
...
func TestPeerListeners(t *testing.T) {
	...
	wg.Add(2)
	go func() {
		inPeer, outPeerErr = peer.NewInboundPeer(&inPeerCfg, inConn)
		wg.Done()
	}()
	...
	for _, test := range tests {
		...
		if err := <-outPeer.QueueMessage(test.msg); err != nil {
			t.Fatal(err)
		}
		select {
		case <-ok:
