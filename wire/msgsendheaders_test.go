...
// prior to version BIP0130Version.
func TestSendHeadersBIP0130(t *testing.T) {
	// Use the protocol version just prior to BIP0130Version changes.
	pver := wire.BIP0130Version - 1
...
// the latest protocol version and decoding with BIP0130Version.
func TestSendHeadersCrossProtocol(t *testing.T) {
	...
	readmsg := wire.NewMsgSendHeaders()
	err = readmsg.BtcDecode(&buf, wire.BIP0130Version)
	if err != nil {
		t.Errorf("decode of MsgSendHeaders failed [%v] err <%v>", buf,
			err)
	}
...
func TestSendHeadersWire(t *testing.T) {
	tests := []struct {
		...
		// Protocol version BIP0130Version+1
		{
			wire.MsgSendHeaders{},
			wire.MsgSendHeaders{},
			[]byte{},
			wire.BIP0130Version + 1,
		},

		// Protocol version BIP0130Version
		{
			wire.MsgSendHeaders{},
			wire.MsgSendHeaders{},
			[]byte{},
