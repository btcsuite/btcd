...
func TestPacketEncodingVectors(t *testing.T) {
	...
	for _, test := range tests {
		...
		xEllswiftOurs := btcec.XSwiftEC(&uEllswiftOursFV, &tEllswiftOursFV)
		...
		xEllswiftTheirs := btcec.XSwiftEC(&uEllswiftTheirsFV, &tEllswiftTheirsFV)
		...
		xShared, err := btcec.EllswiftECDHXOnly(bytesEllswiftTheirs64, privKeyOurs)
		if err != nil {
		...
		if err != nil {
			t.Fatalf("unexpected error when encrypting packet")
		}

		if len(test.outCiphertext) != 0 {
			outCiphertextBytes, err := hex.DecodeString(test.outCiphertext)
			if err != nil {
				t.Fatalf("unexpected error decoding outCiphertext")
			}
