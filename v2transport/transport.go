...
var (
	...
	// errContentLengthExceeded is returned when trying to encrypt more than
...
func (p *Peer) createV2Ciphers(ecdh_secret []byte, initiating bool,
	...
	prk := hkdf.Extract(sha256.New, ecdh_secret, salt)
	...
	if initiating {
		...
		p.sendGarbageTerm = initiatorGarbageTerminator
		...
		p.sendGarbageTerm = responderGarbageTerminator
...
func (p *Peer) InitiateV2Handshake(garbageLen int) error {
	var err error
	p.privkeyOurs, p.ellswiftOurs, err = btcec.EllswiftCreate()
	if err != nil {
		return err
	}
...
func (p *Peer) CompleteHandshake(initiating bool, decoyContentLens []int,
	...
	for i := 0; i < len(decoyContentLens); i++ {
		...
		encPacket, _, err := p.V2EncPacket(decoyContent, aad, false)
		if err != nil {
			return err
		}
