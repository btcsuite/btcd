...
// log is defined in log.go. We use it here.

type packetBit uint8

const (
	// ignoreBitPos is the position of the ignore bit in the packet's header.
	ignoreBitPos packetBit = 7
)

const (
	// garbageSize is the length in bytes of the garbage terminator that each
	// party sends.
	garbageSize = 16
	...
	// headerLen is the length of the header field. It is composed of a single
	// byte with only the ignoreBitPos having any meaning.
	headerLen = 1

	// chachapoly1305Expansion is the difference in bytes between the plaintext
	// and ciphertext when using chachapoly1305. The ciphertext is larger
	// because of the authentication tag.
...
var (
	...
	// errInsufficientBytes is returned when we haven't received enough bytes
	// to populate their ElligatorSwift encoded public key.
	errInsufficientBytes = fmt.Errorf("insufficient bytes received")

	// ErrUseV1Protocol is returned when the initiating peer is attempting to
	// use the V1 protocol.
	ErrUseV1Protocol = fmt.Errorf("use v1 protocol instead")

	// errWrongNetV1Peer is returned when a v1 peer is using the wrong network.
	...
	// errPrefixTooLarge is returned if receivedPrefix is ever too large. This
	// shouldn't happen unless the API is mis-used.
	...
	// ErrShouldDowngradeToV1 is returned when we send the peer our ellswift
	// key and they immediately hang up. This indicates that they don't
	// understand v2 transport and interpreted the 64-byte key as a v1 message
	// header + message. This will (always?) decode to an invalid command and
	// checksum. The caller should try connecting to the peer with the OG v1
	// transport.
...
type Peer struct {
	...
	// sentGarbage is the garbage sent after the public key. This may be up to
	// 4095 bytes.
	sentGarbage []byte
	...
	// sessionID uniquely identifies this encrypted channel. It is currently
	// only used in the test vectors.
	sessionID []byte

	// rw is the underlying object that will be read from / written to in calls
	// to V2EncPacket and V2ReceivePacket.
	rw io.ReadWriter
}
...
func (p *Peer) InitiateV2Handshake(garbageLen int) error {
	...
	log.Debugf("Sending ellswift pubkey and garbage (total_len=%d)", len(data))
...
func (p *Peer) CompleteHandshake(initiating bool, decoyContentLens []int,
	...
	if initiating {
		...
		// If we are the responder, we have already received bytes to compare
		// against the v1 transport protocol's starting bytes. We have to
		// account for these when reading the rest of the 64 bytes off the wire
		// to properly parse the remote's ellswift-encoded public key.
	...
	if err != nil {
		// If we receive an error when reading off the wire and we read zero
		// bytes, then we will reconnect to the peer using v1. There are
		// several different errors that Receive can return that indicate we
		// should reconnect. Instead of special-casing them all, just perform
		// these checks if any error was returned.
		if numRead == 0 && initiating {
			// The peer most likely attempted to parse our 64-byte
			// elligator-swift key as a version message and failed when
			// trying to parse the message header into something valid. In
			// this case, return a special error that signals to the
			// server that we can reconnect with the OG v1 scheme.
			log.Debugf("Received transport error during v2 handshake, " +
				"retying downgraded v1 connection.")
	...
	// Return an error if the garbage terminator was not received after 4 KiB.
...
func (p *Peer) V2EncPacket(contents []byte, aad []byte, ignore bool) ([]byte,
	...
	// Construct the packet's header based on whether or not the peer should
	// ignore this packet (i.e. if this is a decoy packet).
	ignoreNum := 0
	if ignore {
		ignoreNum = 1
	}
