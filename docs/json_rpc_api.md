...
praxisd (Bitcoin-Praxis) provides a [JSON-RPC](http://json-rpc.org/wiki/specification)
API that is fully compatible with the original bitcoind/bitcoin-qt chain RPCs.
Key points:
* Upstream btcd intentionally split wallet and chain into separate processes.
  Bitcoin-Praxis still exposes chain RPCs on praxisd today; an in-process wallet
  (**Bitcoin-Praxis Wallet**, M3) is planned. Until then, wallet RPCs remain
  available via [btcwallet](https://github.com/btcsuite/btcwallet) talking to
  praxisd, or another compatible wallet.
* praxisd is secure by default which means that the RPC connection is TLS-enabled
  by default
* praxisd provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for praxisd RPC. The websocket connection
endpoint is `wss://your_ip_or_domain:8334/ws`.

In addition to the [standard API](#Methods), an [extension API](#WSExtMethods)
has been developed that is exclusive to clients using Websockets.
...
The praxisd RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
praxisd is `wss://your_ip_or_domain:8334/ws`.
...
to a praxisd RPC server:

* **rpcuser** is the full-access username configured for the praxisd RPC server
* **rpcpass** is the full-access password configured for the praxisd RPC server
* **rpclimituser** is the limited username configured for the praxisd RPC server
* **rpclimitpass** is the limited password configured for the praxisd RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the praxisd
  server is configured with.  It is automatically generated by praxisd and placed
  in the praxisd home directory (which is typically `%LOCALAPPDATA%\Praxisd` on
  Windows and `~/.praxisd` on POSIX-like OSes)

**NOTE:** As mentioned above, praxisd is secure by default which means the RPC
...
The praxisd RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
praxisd comes with a separate utility named `praxctl` which can be used to issue
these RPC commands via HTTP POST requests to praxisd after configuring it with the
...
|26|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">praxisd does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
|27|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Until Bitcoin-Praxis Wallet (M3) ships, praxisd does not have an in-process wallet to provide payment addresses, praxisd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|28|[stop](#stop)|N|Shutdown praxisd.|
|29|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|30|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Until Bitcoin-Praxis Wallet (M3) ships, praxisd will only return whether the address is valid or not.|
...
|Returns (verbosity=0)|`"data" (string) hex-encoded bytes of the serialized block`. **Cold-tier (witness-excised) blocks are refused** — use verbosity ≥ 1 (`witness_excised` in JSON) or `--witness-buffer=0` for archival hex.|
...
|Notes|NOTE: Until Bitcoin-Praxis Wallet (M3) ships, chain-only getinfo is returned (no wallet fields). See getinfo in btcwallet for a version which includes wallet information.|
...
|Notes|<font color="orange">Since praxisd does not perform CPU mining by default, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
...
|Notes|NOTE: Until Bitcoin-Praxis Wallet (M3) ships, praxisd does not have an in-process wallet to provide payment addresses, praxisd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">praxisd does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown praxisd.|
|Returns|`"praxisd stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database.<br />The actual checks performed by the `checklevel` parameter is implementation specific.  For praxisd this is:<br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|<font color="orange">praxisd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for praxisd.</font>|
...
The following is an overview of the RPC methods which are implemented by praxisd, but not the original bitcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get bitcoin network praxisd is running on.|None|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `PRXD`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|string|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR PRXD CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get bitcoin network praxisd is running on.|
|Returns|numeric|
...
|Method|version|
|Parameters|None|
|Description|Returns the version of the JSON-RPC API built into this release of praxisd.|
...
|Description|*DEPRECATED, for similar functionality see [loadtxfilter](#loadtxfilter)*<br />Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this praxisd instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
praxisd uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll praxisd for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
a praxisd instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by praxisd when it starts the RPC server and doesn't already
	// have one.
...
The following is an example Go application which uses the
[rpcclient](https://github.com/btcsuite/btcd/tree/master/rpcclient) package to connect with
a praxisd instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/btcsuite/btcd/tree/master/rpcclient) package to connect with
a praxisd instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a praxisd instance,
...
// generated by praxisd when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/praxisd/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The praxisd generated certificate acts as
