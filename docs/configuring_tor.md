...
praxisd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, praxisd supports a hybrid,
...
Configuring praxisd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the praxisd command
line or in the praxisd configuration file.  Typically the Tor proxy address will be
...
By default, praxisd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since praxisd will act as the server.
...
```text
HiddenServiceDir /var/tor/praxisd
HiddenServicePort 8333 127.0.0.1:8333
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring praxisd as a Tor hidden service requires
three flags:
...
```bash
./praxisd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
praxisd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the praxisd command line or in the praxisd
...
```bash
./praxisd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
praxisd provides support for Tor stream isolation by using the `--torisolation`
flag.  This option requires --proxy or --onionproxy to be set.

### Command line example

```bash
./praxisd --proxy=127.0.0.1:9050 --torisolation
```
