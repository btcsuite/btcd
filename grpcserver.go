...
var (
	...
	_ btcrpc.BtcdServer    = (*grpcServer)(nil)
	_ btcrpc.RegtestServer = (*grpcServer)(nil)
)
...
func newGRPCServer(server *server, listeners []net.Listener, tlsConfig *tls.Config) *grpcServer {
	...
	if cfg.RPCMaxConcurrentReqs > 0 {
		opt := grpc.MaxConcurrentStreams(uint32(cfg.RPCMaxConcurrentReqs))
	...
	loggerFunc := func(ctx context.Context, info *tap.Info) (context.Context, error) {
		lastSlash := strings.LastIndex(info.FullMethodName, "/")
		if lastSlash < 0 || lastSlash >= len(info.FullMethodName) {
			return ctx, nil
		}
		method := info.FullMethodName[lastSlash+1:]
		rpcsLog.Debugf("Method called: %v", method)
		return ctx, nil
	}
	serverOpts = append(serverOpts, grpc.InTapHandle(loggerFunc))
...
func (s *grpcServer) GetSystemInfo(ctx context.Context, req *btcrpc.GetSystemInfoRequest) (*btcrpc.GetSystemInfoResponse, error) {
	...
	return &btcrpc.GetSystemInfoResponse{
		...
		TxIndex:   s.server.txIndex != nil,
		AddrIndex: s.server.addrIndex != nil,
	}, nil
}
...
func (s *grpcServer) GetMempool(ctx context.Context, req *btcrpc.GetMempoolRequest) (*btcrpc.GetMempoolResponse, error) {
	...
	for i, txDesc := range mempoolTxs {
		if err := txDesc.Tx.MsgTx().Serialize(&buf); err != nil {
...
// This method assumes that at least either height or hash is provided.
...
func (s *grpcServer) SubmitTransaction(ctx context.Context, req *btcrpc.SubmitTransactionRequest) (*btcrpc.SubmitTransactionResponse, error) {
	...
	// Notify RPC subscriptions of new accepted txs.
	s.HandleNewAcceptedTransactions(acceptedTxs)
	//TODO(stevenroose) forward to getblocktemplate
...
func (s *grpcServer) SubscribeTransactions(stream btcrpc.Btcd_SubscribeTransactionsServer) error {
	...
	go func() {
		for {
			...
			if err != nil {
				if err != io.EOF {
					rpcsLog.Debugf("Error reading from client stream: %v", err)
				}
				close(requests)
				return
			}
