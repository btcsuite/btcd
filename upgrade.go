...
// oldDcrdHomeDir returns the OS specific home directory dcrd used prior to
// version 0.3.3.  This has since been replaced with dcrutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldDcrdHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "dcrd")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".dcrd")
	}
...
// upgradeDBPaths moves the databases from their locations prior to dcrd
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldDcrdHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "dcrd.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "dcrd_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "dcrd_regtest.db"), "regtest")
...
func upgradeDataPaths() error {
	...
	oldHomePath := oldDcrdHomeDir()
	newHomePath := dcrdHomeDir
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		dcrdLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old dcrd.conf into new location if needed.
		...
		if ohpEmpty {
			...
			dcrdLog.Warnf("Not removing '%s' since it contains files "+
