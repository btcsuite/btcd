...
var (
	...
	praxctlHomeDir        = btcutil.AppDataDir("praxctl", false)
	btcwalletHomeDir      = btcutil.AppDataDir("btcwallet", false)
	defaultConfigFile     = filepath.Join(praxctlHomeDir, "praxctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(praxisdHomeDir, "rpc.cert")
...
// config defines the configuration options for praxctl.
//
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(praxctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		// Use config file for RPC server to create default praxctl config
		var serverConfigPath string
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(btcwalletHomeDir, "btcwallet.conf")
		} else {
			serverConfigPath = filepath.Join(praxisdHomeDir, "praxisd.conf")
		}
...
// For this it tries to read the config file for the RPC server (either praxisd or
