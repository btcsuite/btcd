...
// Updater encapsulates the role 'Updater' as specified in BIP174 and BIP0370;
// it accepts Psbt structs and has methods to add fields to the inputs and
// outputs.
type Updater struct {
	Upsbt *Packet
}
...
func (u *Updater) addPartialSignature(inIndex int, sig []byte,
	...
	// PSBT per BIP 370.
	if u.Upsbt.Version == PsbtVersion2 {
		// The sighash type is always the last byte of an ECDSA
		// signature.
		sighashType := txscript.SigHashType(sig[len(sig)-1])

		// If the signature does not use SIGHASH_ANYONECANPAY,
		// the inputs are no longer modifiable. Clear bit 0.
		if sighashType&txscript.SigHashAnyOneCanPay == 0 {
			if u.Upsbt.TxModifiable != nil {
				*u.Upsbt.TxModifiable &= ^FlagInputsModifiable
			}
		}

		// If the signature does not use SIGHASH_NONE, the outputs are
		// no longer modifiable. Clear bit 1. We mask with sigHashMask to
		// ignore the ANYONECANPAY bit when checking the base type.
		if sighashType&sigHashMask != txscript.SigHashNone {
			if u.Upsbt.TxModifiable != nil {
				*u.Upsbt.TxModifiable &= ^FlagOutputsModifiable
			}
		}

		// If the signature uses SIGHASH_SINGLE, the Has SIGHASH_SINGLE
		// flag must be set to true. Set bit 2.
		if sighashType&sigHashMask == txscript.SigHashSingle {
			if u.Upsbt.TxModifiable != nil {
				*u.Upsbt.TxModifiable |= FlagSighashSingle
			}
		}
	}
