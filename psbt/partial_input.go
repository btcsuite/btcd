...
func (pi *PInput) deserialize(r io.Reader, version uint32) error {
	...
	for {
		...
		// (like sequence, txid, output index, and locktimes)
		// must not be present. If they are, the PSBT is invalid.
		switch version {
		case PsbtVersion0:
			switch InputType(keyCode) {
			case PreviousTxidInputType, SpentOutputIndexInputType,
		...
		case SpentOutputIndexInputType:
			if keyData != nil {
				if err := pi.addUnknown(
					byte(keyCode), keyData, value,
				); err != nil {
	...
	switch version {
	case PsbtVersion2:
		if pi.PreviousTxid == nil || !outputIndexSeen {
			return ErrInvalidPsbtFormat
		}
	}
...
func (pi *PInput) serialize(w io.Writer, version uint32) error {
	...
	case PsbtVersion2:
		if pi.PreviousTxid != nil {
			err := serializeKVPairWithType(
				w, uint8(PreviousTxidInputType), nil,
				pi.PreviousTxid,
			)
			if err != nil {
				return err
		...
		err := serializeKVPairWithType(
			w, uint8(SpentOutputIndexInputType), nil,
			outIndexByte[:],
		)
		if err != nil {
			return err
		...
		if pi.HeightLocktime != 0 {
			if pi.HeightLocktime >= 500000000 {
				return ErrInvalidPsbtFormat
			}
